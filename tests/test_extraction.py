...
# --- index tables and scope ------------------------------------------------
def test_index_table_is_excluded_from_context() -> None:
    """ "Index to Consolidated Financial Statements" names every statement and
    holds no figures, so it is the best lexical match for a statement query and
    the worst chunk to spend context on. It appeared twice in Apple's context
    while the actual income statement went unretrieved."""
    from src.extract.extraction_service import is_index_table
    ...
    assert is_index_table(toc) is True


def test_a_real_statement_is_not_mistaken_for_an_index() -> None:
    statement = (
        "|  | September 30, 2023 | September 24, 2022 | | --- | --- | --- | "
        "| Net sales | $383,285 | $394,328 | | Cost of sales | 214,137 | 223,546 |"
    )
    from src.extract.extraction_service import is_index_table

    assert is_index_table(statement) is False


def test_field_anchors_use_or_not_implicit_and() -> None:
    """websearch_to_tsquery ANDs bare terms. A long anchor matched nothing when
    a filer worded the line differently - Apple writes "Cash generated by
    operating activities", not "Net cash provided by". Three of four anchors
    returned zero rows before this."""
    anchors = dict(ExtractionService.FIELD_ANCHORS)

    assert anchors["operating_cash_flow"] == '"operating activities"'
    for phrase in anchors.values():
        assert '"' in phrase, "anchors must be quoted phrases, not bare terms"
    assert " OR " in anchors["total_revenue"]
    assert " OR " in anchors["net_income"]


def test_quarterly_filings_are_excluded_from_annual_scoring() -> None:
    """A 10-Q cannot contain a full-year figure. Scoring one against annual XBRL
    measures the corpus, not the extractor - all three 10-Qs scored 0/4 or 1/4
    on context coverage while every 10-K scored 4/4."""
    from scripts.run_extraction import annual_filings_only

    filings = [
        {"accession": "a", "form": "10-K"},
        {"accession": "b", "form": "10-Q"},
        {"accession": "c", "form": "10-K/A"},
    ]

    kept = {f["accession"] for f in annual_filings_only(filings)}
